...
We need to filter out entries for alters, that have not been named by
the respondents, but are present in the data set since it is structured so that
each _possible_ alter to be named has an entry. This step is technically not necessary
if you are certain that all respondents have named the maximum number of alters.

The logic behind this is to first create a `logical vector` that marks each alter
...
between alters that do not exist. These can be filtered out by excluding all rows/ alter-alter ties
...
5.  Start analysis. Take a look at the [Using Egor](using_egor.html) vignette to learn how to visualize and analyze ego-centered network data with `egor`.

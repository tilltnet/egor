...
a numeric vector with the results. It can be used to apply base R funtions like
...
### Visualisation Wizzard

`egor_vis_wizzard()` starts a Shiny app that allows offers a graphical
interface for adjusting the visualisation parameters of the networks stored
in an egor object.

```
egor_vis_wizzard(egor32)
```
![Wizzard](vis_wizzard.PNG)

## Conversions

In addition to `as_igraph()` and `as_network()` shown in the visualisation 

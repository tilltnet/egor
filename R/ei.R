...
#' @export
EI <- function(object, var_name, ...) UseMethod("EI", object)
...
EI.list <- function(object, aaties, var_name, egoID = "egoID", alterID = "alterID") {
    ...
    rows_to_hm_hts <- function(edge, alters) {
      ...
      hm_ht <- ifelse(
        alters[as.numeric(as.character(alters[[alterID]])) == source_, ][[var_name]] == 
          alters[as.numeric(as.character(alters[[alterID]])) == target_, ][[var_name]], 'HM', 'HT')
      hm_ht
    }
    
    # Calculate group and network EIs.
    lists_to_EIs <- function(aaties, alters, alterID = 'alterID') {
      #print(alters$a0fall)
      ...
      for(i in 1:length(var_levels)) {
        ...
        alters_ids <- alters[alterID][ alters[[var_name]] == var_levels[[i]] , ][[alterID]]
    ...
    alters_df <- do.call(rbind, object)
    na.df <- data.frame(t(c(EI = NA, sc_EI = NA, rep(NA, nlevels(factor(alters_df[[var_name]]))))))
    names(na.df) <- c(names(na.df)[1:2], levels(factor(alters_df[[var_name]])))
    na.df <- data.frame(na.df)
    
    # Invoke mapply on aaties and alters using list_to_EIs.
    EIs <- mapply(lists_to_EIs, aaties, object, SIMPLIFY = F)
    #class(EIs)
    EIs <- lapply(EIs, FUN = function(x) {
      colnames(x) <- colnames(EIs[[1]])
      x
    })
